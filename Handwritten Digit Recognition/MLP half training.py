...
train_targets_test = train_targets[:30000,:]


#one hot encoding
train_targets_final = np.full([60000,10], 0.1, dtype=float)

train_targets_test_final = train_targets_final[:30000,:]

for row in range(len(train_targets_test)):
    index = train_targets[row].astype(int)
    train_targets_test_final[row][index] += 0.8
...
train_images_test = train_images_matrix[:30000,:]
...
def mlpfwd(iter,inputs,hidden_weights,output_weights,train_targets_test_final):
    ...
    error_outputunit = ok * (1-ok) * (train_targets_test_final[iter][:] - ok)
...
for i in range(epochs):
    ...
    train_actual = train_y.transpose().reshape(30000,).astype(int)
    cfm_train = confusion_matrix(train_actual, train_targets_test)
    diagonal_sum_train =  sum(np.diag(cfm_train))
    train_accuracy[i] = (diagonal_sum_train/30000.00)*100
...
image= "Momentum09.png"
plt.title("For half training: 100 hidden units and momentum value of 0.9")
plt.gca().legend()
plt.savefig(image)
plt.show()

print ("CONFUSION MATRIX OF TRAIN SET : For half training: 100 hidden units and Momentum value of 0.9 \n")
print (cfm_train)

print ("CONFUSION MATRIX OF TEST SET : For half training: 100 hidden units and Momentum value of 0.9 \n")
print (test_cm)
